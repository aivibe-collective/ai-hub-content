...
testing templates, and tracking prompt usage.
"""
...
from prompt_management.prompt_management import (
    ...
    update_prompt_template, create_new_version, get_prompt_categories,
    get_prompt_performance_metrics, log_prompt_usage, add_prompt_feedback,
    initialize_default_templates, PromptTemplate
)

from core.google_ai_client import list_models, generate_content
...
def prompt_templates():
    ...
    # Get templates
    prompt_list = get_prompt_templates(category, is_active)

    # Get categories
    categories_list = get_prompt_categories()

    # Get performance metrics
    metrics_list = get_prompt_performance_metrics(category=category)
    ...
    if request.method == 'POST':
        try:
            ...
            if not template_id:
                ...
                categories_list = get_prompt_categories()
                return render_template('prompt_template_form.html', template=template, categories=categories_list)
            ...
            categories_list = get_prompt_categories()
            return render_template('prompt_template_form.html', template=PromptTemplate(), categories=categories_list)
...
def new_version(template_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if not new_id:
                ...
                categories_list = get_prompt_categories()
                return render_template('prompt_template_form.html', template=template, categories=categories_list, is_new_version=True)
    ...
    categories_list = get_prompt_categories()
    return render_template('prompt_template_form.html', template=new_template, categories=categories_list, is_new_version=True)
...
def test_prompt_template(template_id):
    ...
    return render_template('prompt_template_test.html', template=template, models=models_list)
...
def render_prompt_template(template_id):
