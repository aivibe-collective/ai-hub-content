...
try:
    from core.json_fixer import fix_json
except ImportError:
    # If we're running from the root directory
    try:
        from json_fixer import fix_json
    except ImportError:
        # If the JSON fixer is not available, use a simple function
...
class GoogleAIClient:
    ...
    def generate_json(self, prompt, schema=None, temperature=0.2):
        ...
        # Parse JSON using the JSON fixer
        try:
            return fix_json(response_text)
...
def list_models():
    ...
    if not GOOGLE_AI_AVAILABLE:
        # If the Python package is not available, return a default list of models
        return [
            "gemini-1.5-flash",
            "gemini-1.5-pro",
            "gemini-2.0-flash",
    ...
    try:
        ...
        # Return a default list of models if there's an error
        return [
            "gemini-1.5-flash",
            "gemini-1.5-pro",
            "gemini-2.0-flash",
